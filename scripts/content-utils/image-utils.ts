 ...
 */
export function imageEntry(url: string, id: string, variant: number | string = 'default') {
    return {
        id,
        url,
        variant,
 ...
 * @param defaultImage 
 */
export function imageDB(initialEntries: ImageDBEntry[] = [], defaultImage: string = "") {
    const entries: ImageDBEntry[] = [...initialEntries];
    return {
        addImage: (url: string, id: string, variant: number | string = 'default') => {
            entries.push({
                id,
                url,
                variant,
        ...
        getImage: (id: string, variant: number | string = 'default') => {
            ...
            if (entriesWithId.length > 0) {
                const entryWithVariant = entriesWithId.find(entry => entry.variant === variant)
                if (!entryWithVariant) console.warn('Variant not found: ', id, variant)
                return entryWithVariant ? entryWithVariant.url : entriesWithId[0].url
            }
